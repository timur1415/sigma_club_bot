...
async def join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("💸оплатить💸", callback_data="pay")],
                [InlineKeyboardButton('📖прочитать условия📖', url='https://telegra.ph/Usloviya-soglasheniya-i-predostavleniya-uslug-IT-BLANKO-05-10')],
                [InlineKeyboardButton('💬написать в поддержку💬', url='https://t.me/romanenko_vova')],
                [InlineKeyboardButton('🔙в главное меню🔙', callback_data='main_menu')]]
    markup = InlineKeyboardMarkup(keyboard)
    await context.bot.send_photo(
        chat_id=update.effective_chat.id, photo= open('photo/car_hasbik.png', 'rb'), reply_markup=markup
    )
